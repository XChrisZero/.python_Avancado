...
import subprocess # Módulo para executar comandos externos
import sys
...
cmd = [ 'ping', '127.0.0.1' ]
encode = 'utf_8' # Padrão para Linux/Mac
system = sys.platform

# Ajuste do comando para Linux/Mac
...
proc = subprocess_run = subprocess.run(
    cmd, capture_output=True, text=True #encoding= encode  # para poder ver caracteres especiais no Windows
    #shell=False # True para usar o shell do sistema
    )

# Exibindo os resultados
print('Retorno:', proc.returncode) # 0 se sucesso
print("--- Saída do Retorno ---")
print('Args:', proc.args) # Comando e argumentos usados
print('--- Saída Args ---')
print(proc.stdout) # Saída padrão
#print(proc.stdout.decode('utf_8')) # Saída escolhida se não usar text=True
print("--- Fim Saída ---")
print(proc.stderr) # Erro padrão
print("--- Fim Erro ---")
